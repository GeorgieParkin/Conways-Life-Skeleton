    ...
    def countNeighbours(self, cells):
        ...
        for rowdiff in range (-1,2):
            for coldiff in range(-1,2):
                ...
                if 0 <= neighbourRow<len(cells) and 0<=neighbourCol<len(cells):
                    neighbourcount += cells[neighbourRow][neighbourCol].value
        neighbourcount -=self.value
...
class App(tk.Tk):
    ...
    def simulate(self):
        ...
        #      count how many neighbours there are
               thisCount = self.cells[row][col].countNeighbours(self.cells)
               #print(thisCount)
        ...
        for row in range (len(self.cells)):
            for col in range(len(self.cells)):
                self.cells[row][col].setValue(nextGrid[row][col])
